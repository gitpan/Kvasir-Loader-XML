# This Makefile is for the Kvasir::Loader::XML extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/Kvasir/Loader/XML.pm]
#     AUTHOR => q[Claes Jakobsson <claesjac@cpan.org>]
#     LICENSE => q[MIT]
#     NAME => q[Kvasir::Loader::XML]
#     PREREQ_PM => { Test::More=>q[0], Scalar::Util=>q[0], Test::Exception=>q[0], Kvasir=>q[0.07], XML::LibXML=>q[0], Object::Tiny=>q[0] }
#     VERSION_FROM => q[lib/Kvasir/Loader/XML.pm]
...
MAKE = make
FULLEXT = Kvasir/Loader/XML
BASEEXT = XML
PARENT_NAME = Kvasir::Loader
DLBASE = $(BASEEXT)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Kvasir/Loader/XML.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Kvasir/Loader
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Kvasir/Loader
...
TO_INST_PM = lib/Kvasir/Loader/XML.pm

PM_TO_BLIB = lib/Kvasir/Loader/XML.pm \
	blib/lib/Kvasir/Loader/XML.pm
...
DISTNAME = Kvasir-Loader-XML
DISTVNAME = Kvasir-Loader-XML-0.03
...
manifypods : pure_all  \
	lib/Kvasir/Loader/XML.pm
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/Kvasir/Loader/XML.pm $(INST_MAN3DIR)/Kvasir::Loader::XML.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:                Kvasir-Loader-XML' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:             0.03' >> META_new.yml
	$(NOECHO) $(ECHO) 'abstract:            Load Kvasir engine declarations in XML' >> META_new.yml
	$(NOECHO) $(ECHO) 'license:             MIT' >> META_new.yml
	$(NOECHO) $(ECHO) 'generated_by:        ExtUtils::MakeMaker version 6.36' >> META_new.yml
	$(NOECHO) $(ECHO) 'distribution_type:   module' >> META_new.yml
	$(NOECHO) $(ECHO) 'requires:     ' >> META_new.yml
	$(NOECHO) $(ECHO) '    Kvasir:                        0.07' >> META_new.yml
	$(NOECHO) $(ECHO) '    Object::Tiny:                  0' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '    - Claes Jakobsson <claesjac@cpan.org>' >> META_new.yml
...
ppd:
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Load Kvasir engine declarations in XML</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Claes Jakobsson &lt;claesjac@cpan.org&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Kvasir" VERSION="0,07,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Object-Tiny" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' -- \
	  lib/Kvasir/Loader/XML.pm blib/lib/Kvasir/Loader/XML.pm 
